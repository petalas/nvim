...
local colors_dir = vim.fs.normalize(config_dir .. '/colors')

local function normalize(path) return vim.fs.normalize(vim.fn.fnamemodify(path, ':p')) end

local function is_config_file(path)
  path = normalize(path)
  local is_lua = path:sub(1, #lua_dir + 1) == lua_dir .. '/'
  local is_colors = path:sub(1, #colors_dir + 1) == colors_dir .. '/'
  return path == init_file or ((is_lua or is_colors) and path:match '%.lua$')
end

local function notify(message, level) vim.notify(message, level or vim.log.levels.INFO, { title = 'nvim config' }) end
...
local function collect_watch_dirs()
  ...
  local function scan(dir)
    ...
    while true do
      ...
      if filetype == 'directory' then scan(vim.fs.normalize(dir .. '/' .. name)) end
    end
  end
...
local function restart_config(changed_file)
  if vim.g.custom_config_restarting then return end

  vim.g.custom_config_restarting = true
  notify('Restarting after config change: ' .. vim.fn.fnamemodify(changed_file or init_file, ':~:.'))
  local ok, err = pcall(vim.cmd, 'restart')
  if not ok then
    vim.g.custom_config_restarting = false
    notify('Config restart failed: ' .. tostring(err), vim.log.levels.ERROR)
  end
end
...
function M.setup()
  ...
  local function schedule_reload(path)
    if path and not is_config_file(path) then return end
    ...
    state.timer:start(100, 0, function()
      ...
      vim.schedule(function() restart_config(changed_file) end)
    end)
  end
  ...
  vim.api.nvim_create_autocmd('BufWritePost', {
    ...
    callback = function(event)
      if is_config_file(event.file) then schedule_reload(event.file) end
    end,
    desc = 'Restart Neovim after saving config files',
  })
  ...
  for _, dir in ipairs(collect_watch_dirs()) do
    ...
    if watcher then
      local ok = watcher:start(dir, {}, function(err, filename)
        if err then
          vim.schedule(function() notify('Config watcher failed: ' .. tostring(err), vim.log.levels.ERROR) end)
          return
        end
